<?php
// boundaries/CSRRepShortlistedRequestList.php
...
// Handle shortlist/unshortlist actions
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'], $_POST['rID'])) {
    $rID = intval($_POST['rID']);
    $action = $_POST['action'];

    if ($action === 'shortlist') {
        (new CSRRepShortlistRequestController())->ShortlistRequest($rID, $csrID);
    } elseif ($action === 'unshortlist') {
        (new CSRRepUnshortlistRequestController())->UnshortlistRequest($rID, $csrID);
    }
    exit;
}

$shortlistedRIDs = array_column($requests, 'rID'); // just get the rIDs from current $requests
...
if ($priorityFilter !== 'all' || $statusFilter !== 'all' || $categoryFilter !== 'all') {
    $requests = array_filter($requests, function($req) use ($priorityFilter, $statusFilter, $categoryFilter) {
        ...
        $statusMatch   = ($statusFilter === 'all'   || strcasecmp($req['status'], $statusFilter) === 0);
...
<?php if (!empty($requests)): ?>
    <?php foreach ($requests as $req):
        ...
        $icon = $isSelected ? 'boundaries/shortlistIcon.png' : 'boundaries/unselectedShortlistIcon.png';
    ?>
    <tr>
        ...
        <td>
            <button class="activity-button" onclick="viewShortlistedRequest(<?= $req['rID'] ?>)">
    ...
    <?php endforeach; ?>
<?php else: ?>
    <tr><td colspan="10" style="text-align:center;">No shortlisted requests found.</td></tr>
<?php endif; ?>
</tbody>
</table>

<script>
function viewShortlistedRequest(id) {
    window.location.href = 'boundaries/CSRRepViewShortlistedRequest.php?rID=' + id;
}
...
function toggleShortlist(button, csrID, rID) {
    ...
    .then(res => res.text())
    .then(() => location.reload()) // reload after toggle
    .catch(err => {
        console.error(err);
        button.classList.toggle('selected');
