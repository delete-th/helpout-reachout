<?php
// boundaries/CSRRepSearchRequests.php
require_once __DIR__ . '/../controllers/CSRRepSearchRequestsController.php';
require_once __DIR__ . '/../controllers/CSRRepViewRequestController.php';

$CSRRepSearchRequestsController = new CSRRepSearchRequestsController();
...
if (isset($_POST['clearButton'])) {
    $requests = $CSRRepViewRequestController->GetAllRequests();
    $searchInput = '';
} 
elseif (isset($_POST['searchButton']) && !empty(trim($searchInput))) {
    $requests = $CSRRepSearchRequestsController->SearchRequests($searchBy, $searchInput);
} 
else {
    $requests = $CSRRepViewRequestController->GetAllRequests();
}
...
<body>
<h2>Search Person-In-Need Requests</h2>

<form method="post" class="searchContainer">
    <label for="searchBy">Search by:</label>
    <select name="searchBy" id="searchBy">
        <option value="rID" <?php if($searchBy=='rID') echo 'selected'; ?>>Request ID</option>
        <option value="pinID" <?php if($searchBy=='pinID') echo 'selected'; ?>>PIN ID</option>
		<option value="pinName" <?php if($searchBy=='pinName') echo 'selected'; ?>>PIN Name</option>
...
<?php include __DIR__ . '/CSRRepRequestList.php'; ?>
