<?php
// boundaries/DeleteCategoryConfirmationPopUp.php
session_start();
require_once __DIR__ . '/../controllers/PlatformManagerDeleteCategoryController.php';

$cID = isset($_GET['cID']) ? intval($_GET['cID']) : 0;

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $controller = new PlatformManagerDeleteCategoryController();

    if (isset($_POST['confirm']) && $_POST['confirm'] === 'yes') {
        $result = $controller->DeleteCategory($cID);
    } else {
        // Cancelled — redirect back to Platform Manager menu
    ...
    if ($result) {
        ...
        $_SESSION['flash_msg'] = "Failed to delete category. It might be in use.";
    }
    header("Location: ../index.php?action=4_PlatformManager_Menu");
    exit;
}
